...
class ProgressPrinter(cntk_py.ProgressWriter):
    ...
    def __init__(self, freq=None, first=0, tag='', log_to_file=None, rank=None, gen_heartbeat=False, num_epochs=300,
        ...
        Args:
            ...
            eval_freq (`int` or `None`, default `None`): similar to ``freq``, but applies to printing intermediate
              evaluation results.
            eval_first (`int`, default 0): similar to ``first``, but applies to printing intermediate
              evaluation results.
        '''
        if freq is None:
            freq = sys.maxsize
        ...
        if eval_freq is None:
            eval_freq = sys.maxsize

        super(ProgressPrinter, self).__init__(freq, first, eval_freq, eval_first)
    ...
    def on_write_evaluation_update(self, samples, updates, aggregate_metric):
        # Override for ProgressWriter.on_write_evaluation_update.
        self.___write_progress_update(samples, updates, None, aggregate_metric, self.eval_freq, 'Evaluation ')

    def ___write_progress_update(self, samples, updates, aggregate_loss, aggregate_metric, frequency, name):
        samples_since_last = samples[1] - samples[0]
        format_str = ' '
        format_args = []

        if frequency == 0:
            if aggregate_loss is not None:
                avg_loss_since_start = aggregate_loss[1] / samples[1]
                avg_loss_since_last = (aggregate_loss[1] - aggregate_loss[0]) / samples_since_last
                format_str += '{:8.3g}   {:8.3g}   '
                format_args.extend([avg_loss_since_start, avg_loss_since_last])
            else:
                format_str += '{:8s}   {:8s}   '
                format_args.extend(['', ''])

            if aggregate_metric is not None:
                avg_metric_since_start = aggregate_metric[1] / samples[1]
                avg_metric_since_last = (aggregate_metric[1] - aggregate_metric[0]) / samples_since_last
                format_str += '{:8.3g}   {:8.3g}   '
                format_args.extend([avg_metric_since_start, avg_metric_since_last])
            else:
                format_str += '{:8s}   {:8s}   '
                format_args.extend(['', ''])
            ...
            if aggregate_loss is not None:
                ...
                format_args.extend([avg_loss_since_last, samples_since_last])
            ...
            if aggregate_metric is not None:
                ...
                format_args.extend([avg_metric_since_last * 100.0, samples_since_last])
    ...
    def on_write_training_summary(self, samples, updates, summaries, aggregate_loss, aggregate_metric,
        ...
        speed = samples / elapsed_seconds if elapsed_seconds > 0 else 0
        avg_loss = aggregate_loss / samples

        if aggregate_metric is not None:
            avg_metric = aggregate_metric / samples
    ...
    def on_write_evaluation_summary(self, samples, updates, summaries, aggregate_metric, elapsed_milliseconds):
        # Override for ProgressWriter.on_write_evaluation_summary.
        avg_metric = aggregate_metric / samples
        self.___logprint("Finished Evaluation [{}]: Minibatch[1-{}]: metric = {:0.2f}% * {};".format(
            summaries, updates, avg_metric * 100.0, samples))
...
class TensorBoardProgressWriter(cntk_py.ProgressWriter):
    ...
    def on_write_training_update(self, samples, updates, aggregate_loss, aggregate_metric):
        ...
        self.write_value('minibatch/avg_loss', avg_loss, updates[1])

        avg_metric = (aggregate_metric[1] - aggregate_metric[0]) / (samples[1] - samples[0])
        self.write_value('minibatch/avg_metric', avg_metric, updates[1])
    ...
    def on_write_training_summary(self, samples, updates, summaries, aggregate_loss, aggregate_metric,
        ...
        self.write_value('summary/avg_loss', aggregate_loss / samples, summaries)
        self.write_value('summary/avg_metric', aggregate_metric / samples, summaries)

    def on_write_evaluation_summary(self, samples, updates, summaries, aggregate_metric, elapsed_milliseconds):
        # Override for BaseProgressWriter.on_write_evaluation_summary().
        if self.total_training_updates() != 0:
            ...
            self.write_value('minibatch/eval_avg_metric', aggregate_metric / samples, self.total_training_updates())
        else:
            self.write_value('summary/eval_avg_metric', aggregate_metric / samples, self.summaries)
